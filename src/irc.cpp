...
// Copyright (c) 2011-2012 gicoin Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #gicoinTEST0\r");
            Send(hSocket, "WHO #gicoinTEST0\r");
        } else {
            // randomly join #gicoin00-#gicoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #gicoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #gicoin%02d\r", channel_number).c_str());
        }
