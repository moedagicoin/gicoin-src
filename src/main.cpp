...
const string strMessageMagic = "gicoin Signed Message:\n";
...
{
    // gicoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetSpacing = 0.3 * 60;
...
{
    // gicoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0x92, 0xc4, 0xaa, 0xef };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("gicoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("gicoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static gicoinMiner(CWallet *pwallet)
{
    printf("gicoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("gicoin-miner");
    ...
    try { loop {
        ...
        printf("Running gicoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&gicoinMiner, pwallet));
}
